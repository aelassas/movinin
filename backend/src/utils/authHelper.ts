...
export const validateAccessToken = async (
  socialSignInType: movininTypes.SocialSignInType,
  token: string,
  email: string
): Promise<boolean> => {
  if (!token) {
  ...
  try {
    switch (socialSignInType) {
      case movininTypes.SocialSignInType.Apple:
        return await verifyAppleToken(token, email)

      case movininTypes.SocialSignInType.Google:
        return await verifyGoogleToken(token, email)

      case movininTypes.SocialSignInType.Facebook:
