...
routes.route(routeNames.createImage).post([authJwt.verifyToken, authJwt.authAgency, multer({ storage: multer.memoryStorage() }).single('image')], locationController.createImage)
routes.route(routeNames.updateImage).post([authJwt.verifyToken, authJwt.authAgency, multer({ storage: multer.memoryStorage() }).single('image')], locationController.updateImage)
