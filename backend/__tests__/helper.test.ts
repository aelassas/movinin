...
describe('Test validateAccessToken', () => {
  it('should test validateAccessToken', async () => {
    ...
    // test failure
    expect(await helper.validateAccessToken('unknown' as movininTypes.SocialSignInType, 'token', email)).toBeFalsy()

    // test success (facebook)
    let payload = {}
    let token = await encryptJWT(payload)
    expect(await helper.validateAccessToken(movininTypes.SocialSignInType.Facebook, token, email)).toBeTruthy()

    // test success (apple)
    payload = { email }
    token = await encryptJWT(payload)
    expect(await helper.validateAccessToken(movininTypes.SocialSignInType.Apple, token, email)).toBeTruthy()
    ...
    await jest.isolateModulesAsync(async () => {
      ...
      payload = { data: { email } }
      token = await encryptJWT(payload)
      expect(await helper.validateAccessToken(movininTypes.SocialSignInType.Google, token, email)).toBeTruthy()
      jest.resetModules()
      jest.clearAllMocks()
    })
  })
